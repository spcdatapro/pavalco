(function(){
    ...
    trandirectactrl.controller('tranDirectaCtrl', ['$scope', 'directaSrvc', 'authSrvc', 'empresaSrvc', 'DTOptionsBuilder', 'toaster', '$confirm', 'cuentacSrvc', 'detContSrvc', 'contratoSrvc', function($scope, directaSrvc, authSrvc, empresaSrvc, DTOptionsBuilder, toaster, $confirm, cuentacSrvc, detContSrvc, contratoSrvc){
        ...
        $scope.losDetCont = [];
        $scope.elDetCont = {debe: 0.0, haber: 0.0, idcontrato: '0'};
        $scope.lasCtasMov = [];
        $scope.origen = 4;
        $scope.directastr = '';
        $scope.contratos = [];

        $scope.dtOptions = DTOptionsBuilder.newOptions().withPaginationType('full_numbers').withBootstrap().withOption('responsive', true).withOption('ordering', false).withOption('fnRowCallback', rowCallback);
        ...
        authSrvc.getSession().then(function(usrLogged){
            if(parseInt(usrLogged.workingon) > 0){
                empresaSrvc.getEmpresa(parseInt(usrLogged.workingon)).then(function(d){
                    ...
                    contratoSrvc.lstContratos($scope.objEmpresa.id).then(function(d){ $scope.contratos = d; });
                });
            }
        });
        ...
        $scope.getDetCont = function(iddirecta){
            $scope.elDetCont = {debe: 0.0, haber: 0.0, idcontrato: '0', objCuenta: undefined};
