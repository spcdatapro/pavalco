...
class NumberToLetterConverter {
  ...
  private $MONEDAS = array(
    ...
    array('country' => 'Guatemala', 'currency' => 'GTQ', 'singular' => 'QUETZAL', 'plural' => 'QUETZALES', 'symbol', 'Q')
  );
    ...
    public function to_word_int($number, $miMoneda = null) {
        if (strpos($number, $this->decimal_mark) === FALSE) {
            $convertedNumber = array(
                $this->convertNumber($number, $miMoneda, 'entero')
            );
        ...
        if ($miMoneda !== null) {
            try {
                ...
                if($type === 'entero'){
                    ...
                    ($number < 2 ? $moneda = 'CENTAVO' : $moneda = 'CENTAVOS');
                };
            } catch (Exception $e) {
                echo $e->getMessage();
                return;
