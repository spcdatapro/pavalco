...
$app->get('/lstpagos/:idempresa/:flimite/:idmoneda', function($idempresa, $flimite, $idmoneda){
    ...
    $query.= "(a.totfact - (a.isr + IFNULL(c.montopagado, 0.00))) AS saldo, 0 AS pagar, d.simbolo AS moneda, a.tipocambio, a.idmoneda, a.isr ";
    ...
    for($y = 0; $y < $cantProvs; $y++){
        ...
        for($z = 0; $z < $cantPagos; $z++){
            ...
            if((int)$d[$z]->idproveedor == $idprovs[$y]){
                ...
                if($nombreProveedor == ''){ $nombreProveedor = $d[$z]->proveedor; };
        ...
        $query = "INSERT INTO tranban(idbanco, tipotrans, fecha, monto, beneficiario, concepto, numero, origenbene, idbeneficiario, tipocambio) ";
        $query.= "VALUES(".$objBanco->idbanco.", 'C', '".$objBanco->fechatranstr."', ".$totAPagar.", '".$nombreProveedor."', ";
        $query.= "'Pago de factura(s) ".$qFacturas."', (".$getCorrela."), 1, $idprovs[$y], $objBanco->tipocambio)";
        //echo $query.'<br/><br/>';
        $db->doQuery($query);
        $lastid = $db->getLastId();
