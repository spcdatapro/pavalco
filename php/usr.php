...
$app->get('/lstperfiles/:usrlogged', function($usrlogged){
    $db = new dbcpm();
    $conn = $db->getConn();
    $query = "SELECT id, nombre, usuario, correoe FROM usuario ";
    $query.= (int)$usrlogged <> 1 ? "WHERE id <> 1 " : "";
    $query.= "ORDER BY nombre";
    $perfiles = $conn->query($query)->fetchAll(5);
    print json_encode($perfiles);
});

$app->get('/permisos/:idusr/:usrlogged', function($idusr, $usrlogged){
    $db = new dbcpm();
    $conn = $db->getConn();
    ...
    $query.= (int)$usrlogged <> 1 ? 'AND b.id NOT IN(1, 2) ' : '';
