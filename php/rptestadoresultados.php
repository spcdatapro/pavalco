<?php
//--------------------------------------------------------------------
//class forTest{ public $fdelstr = ''; public $falstr = ''; public $idempresa = 1; }
//--------------------------------------------------------------------
...
$app->post('/rptestres', function(){
    ...
    foreach($inggas AS $ig){
        ...
        foreach($ctasing as $ing){
            ...
            foreach($inicianCon as $ini){
                ...
                $query.= "FROM rptestadoresultados WHERE ingresos = ".$ig." AND LENGTH(codigo) = 6";
                $db->doQuery($query);
            }
        }
        $query = "INSERT INTO rptestadoresultados(idcuenta, codigo, nombrecta, tipocuenta, ingresos, saldo, estotal) ";
        $query.= "SELECT 0, '', 'Total de ".($ig == 1 ? "ingresos" : "gastos")."', 1, ".$ig.", SUM(saldo), 1 ";
    ...
    $query = "INSERT INTO rptestadoresultados(idcuenta, codigo, nombrecta, saldo) VALUES(";
    $query.= "0, '', '".($estado >= 0 ? "Ganancia" : "Perdida")." del ejercicio', ".$estado;
    $query.= ")";
    $db->doQuery($query);
