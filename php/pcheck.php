...
//Seleccionar datos de impresion
$query = "SELECT margleft, margtop, margright, fontsize, fecinien, fechawsize, fechahsize FROM confpcheck WHERE id = 1";
$ddi = $db->getQuery($query)[0];
...
$pdf->Cell(1.3 * $conv);
$pdf->Cell(4.4 * $conv, 0.7 * $conv, $cheque->montostr, $borde, 2);
$pdf->Cell(-10.3 * $conv);
$pdf->Cell(14 * $conv, 0.65 * $conv, iconv('UTF-8', 'windows-1252', $cheque->beneficiario), $borde, 2);
$pdf->Cell(-0.4 * $conv);
$pdf->Cell(14.5 * $conv, 0.65 * $conv, $n2l->to_word_int($cheque->monto), $borde, 2);
$pdf->Cell(1.5 * $conv, 0.65 * $conv, '', 0, 2);
$pdf->Cell(1.5 * $conv);
